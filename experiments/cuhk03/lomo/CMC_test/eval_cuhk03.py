...
def generateScoreList(net,probes,gallerys,map_lomo, deep = True):
    ...
    for probeIdx in range(len(probes)):
        ...
        outScore = net.blobs['softmax_score'].data[:,(0,1)]#+net.blobs['softmax_score_lomo'].data[:,(0,1)]   #softmax_score[0] and softmax_score[1]
...
def calCMC(net,set_no,map_lomo,deep=True,rand_times=10):
    ...
    DATA_DIR= 'dataset/cuhk03_detect/'
...
def main():
    ...
    for iter_num in range(5000, 35000, 5000):
        ...
        for set_no in test_list:
            #init net
            MODEL_FILE = './experiments_py_lomo/cuhk03/lomo/deploy_lomo.prototxt'
            PRETRAINED = './models/lomo/cuhk03/lomo/30000_set02_iter_%d.caffemodel'%iter_num
            net = None
            net = caffe.Classifier(MODEL_FILE, PRETRAINED,caffe.TEST)
            #caculate CMC
            cmc=calCMC(net,set_no,map_lomo,deep=False,rand_times=50)
            cmc_list.append(cmc)
    ...
    save_root = 'experiments_py_lomo/cuhk03/lomo/CMC_test/'
    save_mat_path = save_root + 'cuhk03_set02_yqCMC_100round_30000.mat'
    import scipy.io as sio
    sio.savemat(save_mat_path,{'ours':CMC_DIC})

    save_path = save_root + 'cuhk03_set02_yqCMC_100round_30000.png'
